...
- **进度保存弹窗显示 `undefined`？**
    ...
    已核对的播放器在心跳请求返回 `status=-2` 时直接显示 `error_desc`。响应缺少这个字段就会显示 `undefined`，随后平台清除播放器。4.0.2 会依次选择 `error_desc`、`message`、`msg`，均缺失时显示状态码。后台拒绝保存的原因应以实际响应说明为准。

- **更新后外层页面和下一节表现不同？**

    在已有脚本条目中更新至 4.0.2，保存后刷新最外层课程页，使外层和播放器 iframe 加载同一版本。页面内切集通常只重载 iframe。验收和诊断方法见[自动连播验证](docs/auto-advance.md)。
