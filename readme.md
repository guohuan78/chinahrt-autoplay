...
- 整体回退到经过线上验证的旧版函数式架构（连播收尾流程与原版一致），此前重构版多轮修复仍无法在部分地区的站点上完成自动连播
- 在旧版架构上叠加三处增强：
  1. 自动播放守护：每秒重试 `videoPlay()`，直到视频真正开始（修复自动跳转后不能自动播放）
  2. SPA 播放页支持：`#/v_video` 路由识别为播放页 + `hashchange` 兜底启动，单页应用内切换课程也能自动播放
  3. 收尾兜底：学习记录接口卡住或报错时最多等 5 秒必跳转下一节
- 播放列表存储格式双向兼容（新版/旧版脚本添加的课程可混用），`sectionId` 匹配不再受数字/字符串类型影响

### v3.1.3-fix.7

- 补全 SPA 播放页路由支持：原代码课程详情页路由切换后不会初始化播放页，且路由回调首行调用已删除的方法抛错导致整个回调失效
- 播放结束流程加固：学习记录接口卡住或异常时最多等 5 秒必然跳转下一节，不再停留在已播完的页面
- 播放器事件去重注册，SPA 内播放器实例轮换后自动重新初始化
