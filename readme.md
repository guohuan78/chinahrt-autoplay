...
- 修复播完后无法自动播放下一个视频的问题：视频结束后先提交学习记录（`takeRecord`，`isEnd: true`）并清理播放器，提交完成后再跳转下一节；记录接口异常时也会照常跳转

### v3.1.3-fix.5

- 修复视频播完后无法从播放列表自动移除的问题：播放页地址与列表地址的参数形式不一致时 URL 匹配失效；改为优先用播放器自身的 `sectionId`（与提交学习记录同源）匹配，URL 签名匹配仅作兜底
